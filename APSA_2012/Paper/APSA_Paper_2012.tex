...
Figure~\ref{3d} shows the median CRPS recorded for differing values of the ratio $\frac{K}{N_T}$, and $c$. The 3D plot clearly shows that while CRPS generally still increases with higher values of $\frac{K}{N_T}$, including $c$ within the EBMA algorithm can help. 
...
$c$ as the ratio of $n_T$ to $K$ increases. As a starting value, we
recommend $c=0.05$, however the correct value of $c$ may value from application to application. In our experience based on the simulation, as well as cross-validation studies during the applications presented below we feel a $c$ value at $0.05$ was the best choice. Generally speaking in choosing $c$ the researcher has to trade-off between possible overweighting certain model based on the performance in short calibration periods, or moving too far into the direction of a simple average. A relatively small $c$ to us seems the most values. It may be preferable to many researchers to choose the value of $c$
...
The rows of the table show the number of metrics by which EBMA outperforms components in each column, while columns show the number of forecasts made by these models. The table is filled with the share of the total number of models 
falling in this cell. Thus for the top left cell EBMA was better on at least 7 out of eight metrics for models making between 1 and 10 predictions, approximately 20\% of models belong in this category. 
