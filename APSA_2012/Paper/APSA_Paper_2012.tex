...
The introduction of the ``wisdom of crowds'' parameter to the baseline
...
the number of component forecasts is large.  We stated above that
these circumstances increase the miss-estimation of component model
weights and decrease the predictive performance of EBMA.
...
applications of the method.  We begin with a simulation that
...
apply our method to, first, the prediction of the US unemployment rate
using the forecasts from the Survey of Professional Forecasters, and,
second, to the prediction of the 2012 US presidential election.

\subsection{Simulation study} 

In this section, we conduct a simulated study of our small-sample EBMA
model.  These simulations serve three purposes.  First, they
demonstrate the challenges presented to ensemble forecasting when
...
modified EBMA algorithm ameliorates these difficulties.  Third, this
exercise provides some guidance regarding the selection of $c$ for the
applied examples below.

The simulations are designed to reflect the ``best possible''
situation for EBMA.  The distribution of outcomes is drawn precisely
from the mixture distribution shown in Equation~(\ref{pdf}) where
$\sigma^2 = 1$.  Moreover, we assume that the true data generating
...
Our goal is to compare the accuracy of the baseline EBMA ($c=0$)
...
the true data generating process increase.  That is, as the number of
model parameters that \textit{must} be correctly estimated to make
accurate predictions increase, the quality of the forecast goes down.
...
for differing values of $c$. The results are shown in
Figure~\ref{simplot2}. For the purposes of clarity, we focus on only
three subsets from the simulations: $n_T=3, K=15$, $n_T=5, K=9$,
$n_T=35, K=5$.  While this is far from a complete analysis of the
simulated data, it does serve the limited purposes of demonstrating
that, in some circumstances, the ``wisdom of crowds'' parameter aids
prediction.
...
One prominent example of forecasting that combines all of these issues
is the prediction of U.S. Presidential elections.  Table 1 represents
nearly the entirety of scholarly forecasts which produced more than
one out-of-sample forecast for elections in the 20th century prior to
the 2012 election.\footnote{ See, for example \citet[][]{Fair:2009,
...
Finally, we note that in future research, we intend to (1)
compare alternative methods of handling missing data (2) discuss how
to select window of time for calibration, (3) conduct
simulation studies to explore settings for $c$ parameter, and (4) test
